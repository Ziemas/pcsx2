...
private:
#ifdef _WIN32
	bool GetWin32SelectedAdapter(const char* name, PIP_ADAPTER_ADDRESSES adapter, std::unique_ptr<IP_ADAPTER_ADDRESSES[]>* buffer);
	bool GetWin32AutoAdapter(PIP_ADAPTER_ADDRESSES adapter, std::unique_ptr<IP_ADAPTER_ADDRESSES[]>* buffer);
#elif defined(__POSIX__)
	bool GetIfSelectedAdapter(const char* name, ifaddrs* adapter, ifaddrs** buffer);

...
std::vector<AdapterEntry> SocketAdapter::GetAdapters()
{
	...
	autoEntry.type = NetApi::Sockets;
#ifdef _WIN32
	autoEntry.name = L"Auto";
	autoEntry.guid = L"Auto";
#else
	autoEntry.name = "Auto";
	autoEntry.guid = "Auto";
#endif
	...
	do
	{
		...
		{
			AdapterEntry entry;
			entry.type = NetApi::Sockets;
			entry.name = std::wstring(pAdapterInfo->FriendlyName);
			//Convert char to wstring
			wchar_t wGuid[sizeof(config.Eth)] = {0};
			mbstowcs(wGuid, pAdapterInfo->AdapterName, sizeof(config.Eth) - 1);
			entry.guid = std::wstring(wGuid);
...
SocketAdapter::SocketAdapter()
{
	...
	{
		foundAdapter = GetWin32SelectedAdapter(config.Eth, &adapter, &buffer);
	...
	if (strcmp(config.Eth, "Auto") != 0)
	{
		foundAdapter = GetIfSelectedAdapter(config.Eth, &adapter, &buffer);
...
#ifdef _WIN32
bool SocketAdapter::GetWin32SelectedAdapter(const char* name, PIP_ADAPTER_ADDRESSES adapter, std::unique_ptr<IP_ADAPTER_ADDRESSES[]>* buffer)
{
	int neededSize = 128;
	std::unique_ptr<IP_ADAPTER_ADDRESSES[]> AdapterInfo = std::make_unique<IP_ADAPTER_ADDRESSES[]>(neededSize);
	ULONG dwBufLen = sizeof(IP_ADAPTER_ADDRESSES) * neededSize;
...
#elif defined(__POSIX__)
bool SocketAdapter::GetIfSelectedAdapter(const char* name, ifaddrs* adapter, ifaddrs** buffer)
{
	ifaddrs* adapterInfo;
	ifaddrs* pAdapter;
